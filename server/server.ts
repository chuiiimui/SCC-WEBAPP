...
interface WikidataBinding {
  ...
  date?: { value: string };
  legal_citation?: { value: string };
  courtLabel?: { value: string };
  majority_opinionLabel?: { value: string };
...
// Search Index & Cache
// --------------------
const CASE_INDEX: Map<string, CaseResult[]> = new Map();
const cacheWithTTL: Map<string, CacheEntry> = new Map();
...
// Utility Functions
// --------------------
function buildSparqlQuery(countryId: string, courtId: string): string {
  return `
...
function mapWikidataResult(item: WikidataBinding, country: string): CaseResult {
...
function setCacheEntry(key: string, data: any): void {
  if (cacheWithTTL.size >= MAX_CACHE_SIZE) {
    ...
    if (firstKey) cacheWithTTL.delete(firstKey);
  }
  cacheWithTTL.set(key, { data, timestamp: Date.now() });
}

function calculateScore(caseItem: CaseResult, params: SearchParams): number {
  let score = 0;
  ...
  if (caseItem.title.toLowerCase().includes(keyword)) score += 10;

  // Description match
  if (caseItem.description.toLowerCase().includes(keyword)) score += 5;

  // Citation match
  if (caseItem.citation.toLowerCase().includes(keyword)) score += 7;

  // Judges match
  if (caseItem.judges.toLowerCase().includes(keyword)) score += 4;

  // Court match
  if (caseItem.court.toLowerCase().includes(keyword)) score += 3;
...
async function buildIndex(country: string) {
  ...
  try {
    ...
    const cases = bindings.map((item: WikidataBinding) => mapWikidataResult(item, country));
...
function searchCases(params: SearchParams, country: string): CaseResult[] {
      ...
      if (keyword) {
        ...
        matches =
          ...
          keywordTokens.some((token) =>
            [c.title, c.description, c.court, c.citation, c.judges]
              .map((f) => (f || "").toLowerCase())
...
app.get("/api/health", (_req, res) => {
  const indexStatus = Object.fromEntries(
    Array.from(CASE_INDEX.entries()).map(([country, cases]) => [country, cases.length])
  );
>>>>>>> 4f727c7 (...//D//)
  res.json({
    status: "healthy",
...
// --------------------
app.get("/search", async (req: Request, res: Response) => {
  const userQuery = normalize(req.query.q as string);
  const countryParam = normalize(req.query.country as string) || "ghana";
...
=======
  ...
  try {
    ...
    const allCases: CaseResult[] = (data as any).results.bindings
      .map((item: WikidataBinding) => mapWikidataResult(item, countryParam))
      .filter((c: CaseResult) =>
        !userQuery ||
        [c.title, c.description, c.court, c.citation, c.judges].some((f) =>
          (f || "").toLowerCase().includes(userQuery)
        )
      );
...
app.get("/filter", (req: Request, res: Response) => {
  ...
  const caseType = req.query.type as string;
  const countryParam = normalize(req.query.country as string) || "ghana";
  const page = Math.max(1, parseInt(req.query.page as string) || 1);
...
app.get("/api/recent-searches", (req: Request, res: Response) => {
  const searches = (req.session.recentSearches as RecentSearch[]) || [];
...
=======
  ...
  try {
    ...
    const translationsMap = new Map<string, any>();
    bindings.forEach((item: WikidataBinding) => {
      const cId = item.item?.value.split("/").pop() || "Unknown";
      const caseTitle = item.itemLabel?.value || "Unknown Case";
